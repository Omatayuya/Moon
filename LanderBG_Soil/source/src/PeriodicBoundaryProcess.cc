...
PeriodicBoundaryProcess::PeriodicBoundaryProcess(const G4String& processName, G4ProcessType type, G4bool per_x, G4bool per_y, G4bool per_z)
...
G4VParticleChange* PeriodicBoundaryProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
	...
	if(aTrack.GetTrackID()!=1){ //not primary particle
		return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
	}
	...
	if(verboseLevel>0){
		...
		G4cout << " ParentID : " << aTrack.GetParentID() << "  TrackID : " << aTrack.GetTrackID() << G4endl;
		G4cout << " Position : " << aTrack.GetPosition() << "  aTrack Energy : " << G4BestUnit(aTrack.GetKineticEnergy(), "Energy") << G4endl;
	}

	//avoid trapped particles at boundaries by testing for minimum step length
	if(aTrack.GetStepLength()<=0.5*fkCarTolerance){
	...
	G4bool valid = false;
	// Use the new method for Exit Normal in global coordinates, which provides the normal more reliably.
	// get from the real-world navigator (process not applied to parallel worlds)
	fTheGlobalNormal = G4TransportationManager::GetTransportationManager()->GetNavigatorForTracking()->GetGlobalExitNormal(theGlobalPoint, &valid);

	if(valid) fTheGlobalNormal = -fTheGlobalNormal;
	else{
		G4cout << "global normal " << fTheGlobalNormal << G4endl;
		G4ExceptionDescription ed;
		ed << " PeriodicBoundaryProcess::PostStepDoIt(): The Navigator reports that it returned an invalid normal" << G4endl;
		G4Exception("G4PeriodicBoundaryProcess::PostStepDoIt", "PerBoun01", FatalException, ed, "Invalid Surface Normal - Geometry must return valid surface normal");
	}

	G4bool isWrongDirection = fOldMomentum*fTheGlobalNormal>0.;
	if(isWrongDirection){
		if(verboseLevel>0){
			G4cout << "ftheGlobalNormal points in a wrong direction." << G4endl;
			G4cout << "Invalid Surface Normal - Geometry must return valid surface normal pointing in the right direction" << G4endl;
		}
	...
	if(thePostPV==nullptr){
		G4ExceptionDescription ed;
		ed << " PeriodicBoundaryProcess::PostStepDoIt(): thePostPV == nullptr" << G4endl;
		G4Exception("G4PeriodicBoundaryProcess::PostStepDoIt", "PB14", FatalException, ed, "Invalid thePostPV");
	}

	auto lvol = thePostPV->GetLogicalVolume();
	if(lvol == nullptr){
		G4ExceptionDescription ed;
		ed << " PeriodicBoundaryProcess::PostStepDoIt(): lvol == nullptr" << G4endl;
		G4Exception("G4PeriodicBoundaryProcess::PostStepDoIt", "PB12", FatalException, ed, "Invalid lvol");
	}
	if(verboseLevel>0) G4cout << "Post step logical " << lvol->GetName() << G4endl;

	G4LogicalVolume* dlvol = nullptr;
	if(lvol->GetNoDaughters()>0){
		if(verboseLevel>0) G4cout << "eldest daughter " << lvol->GetDaughter(0)->GetName() << G4endl;
	...
	if(dlvol&&dlvol->IsExtended()){
		...
		if(!on_plane){
			G4ExceptionDescription ed;
			ed << " G4PeriodicBoundaryProcess::ostStepDoIt(): The particle is not on a surface of the cyclic world" << G4endl;
			G4Exception("G4PeriodicBoundaryProcess::PostStepDoIt", "Periodic01", FatalException, ed, "Periodic boundary process must only occur for particle on periodic world surface");
		}else{
			...
			if(on_a_periodic_plane){
				...
				if(verboseLevel>0) G4cout << "Global normal " << fTheGlobalNormal << G4endl;

				// translate a component of the position vector according to which plane we are on
				if(on_x_and_periodic) fNewPosition.setX(-fNewPosition.x());
				else if(on_y_and_periodic) fNewPosition.setY(-fNewPosition.y());
				else if(on_z_and_periodic) fNewPosition.setZ(-fNewPosition.z());
				else G4cout << "global normal does not belong to periodic plane!!" << G4endl;
				...
				// Inform the navigator that the previous Step calculated by the geometry was taken in its entirety.
				gNavigator->SetGeometricallyLimitedStep();

				// Search the geometrical hierarchy for the volumes deepest in the hierarchy containing the point in the global coordinate space.
				gNavigator->LocateGlobalPointAndSetup(fNewPosition, &fNewMomentum, false, false);  // do not ignore direction

				// Calculate the isotropic distance to the nearest boundary from the specified point in the global coordinate system.
				...
				if(pTrajectory) pTrajectory->AppendStep(pStep);
			}
		}
	}
...
G4double PeriodicBoundaryProcess::GetMeanFreePath(const G4Track&, G4double, G4ForceCondition* condition)
{
	*condition = Forced;
...
{
	if(fStatusMessages.count(fTheStatus)>0) G4cout << PeriodicBoundaryProcess::fStatusMessages[fTheStatus] << G4endl;
}
