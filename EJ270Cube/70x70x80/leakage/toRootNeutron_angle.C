...
int toRootNeutron_angle(TString strTarget = "H_0ppm"){
	gROOT->Reset();
	gROOT->SetBatch();
	...
	vector<TString> vStrPrimary{"01-H", "02-He"};
	const UInt_t nGCR = vStrPrimary.size(); //number of sources (max=2: Proton, Helium)

	const Double_t sensArea = pow(1414.2,2); //sensitive surface area (cm^2)
	const Double_t irrArea = pow(2000,2); //irradiation surface area (cm^2) for Proton, Helium
	...
	for(size_t i=0; i<vStrPrimary.size(); i++){
		...
		ssInput << "/extra1/kusano/G4work/Moon/GCR-Surface/Homogeneous/Shielding/FLM_273K/" << strTarget << "/" << vStrPrimary.at(i) << "/" << strPhi << "/results.root";
		TString filename = ssInput.str();
		ifstream ifs(filename);
		if(ifs.fail()){ cerr << "--- File not found" << endl; return 1; }
		...
		vector< TTreeReaderArray<double> > vaReaderEnergy, vaReaderMomentumZ;
		for(const auto& el : vParticle){
			...
			TTreeReaderArray<double> aMomentumZ(reader, el.name+"MomentumZ");
	...
	if(true){
		stringstream ssRoot;
		ssRoot << "Neutron_" << strPhi << "_" << strTarget << "_angle.root";
