...
void MyParallelWorld::Construct()
{
    ...
    // Imagenary Cd layer
    constexpr G4double layerSpacing = 5 * mm;
    constexpr G4double layerT = 10 * um;

    G4int copyNb_cdLayer = 2000;
    G4int nCdLayer = 0;
    for (G4double z = -hZ ; z <= hZ; z += layerSpacing)
    {
        auto box = new G4Box("Box_CdLayer_" + std::to_string(nCdLayer),
                             hX, hY, layerT / 2.0);

        auto lv = new G4LogicalVolume(box, nullptr, "LV_CdLayer_" + std::to_string(nCdLayer));
        lv->SetVisAttributes(G4VisAttributes(G4Colour::Yellow()));

        G4ThreeVector pos = SensorCenter + G4ThreeVector(0, 0, z);
        new G4PVPlacement(nullptr, pos, lv, "PV_CdLayer_" + std::to_string(nCdLayer),
                          ghostLogical, false, ++copyNb_cdLayer, true);
        ++nCdLayer;
    }
}
...
void MyParallelWorld::ConstructSD()
{
    ...
    auto cdLayerSD = new B2TrackerSD("ParallelWorld/CdLayerSD",
                                     "CdLayerHitsCollection");
    sdMan->AddNewDetector(cdLayerSD);

    const G4double hZforSD = fDetector->GetSensorHalfSize().z();
    constexpr G4double layerSpacingForSD = 5 * mm;
    G4int nCdLayerForSD = 0;
    for (G4double z = -hZforSD ; z <= hZforSD; z += layerSpacingForSD)
    {
        SetSensitiveDetector("LV_CdLayer_" + std::to_string(nCdLayerForSD), cdLayerSD, true);
        ++nCdLayerForSD;
    }
}
