   ...
   fDetectorConstruction(Det)
{
  ...
  fFilmMatCmd = new G4UIcmdWithAString("/B2/det/setFilmMaterial",this);
  fFilmMatCmd->SetGuidance("Select Material of the Film.");
  fFilmMatCmd->SetParameterName("choice",false);
...
void B2aDetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  if( command == fFilmMatCmd )
   { fDetectorConstruction->SetFilmMaterial(newValue);}
